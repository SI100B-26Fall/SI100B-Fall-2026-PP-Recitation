...
- 官网下载安装包：
  ...
  - 下载对应系统的安装包（Windows 是 `.exe`，macOS 是 `.dmg`，Linux 可用 `.deb` / `.rpm` 等）
    ...
    <img src="images/vscode_dmg.png" width="50%" style="display: block; margin: 0 auto;">
- 或者用自带的包管理器（注意网络环境，可以自行搜索）
  - macOS 可用 `brew` 或者自带应用商店
    - `brew`安装：`/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"`
    - 通过`brew`安装VS Code：`brew install visual-studio-code`
  - Windows 可以用 `winget`
